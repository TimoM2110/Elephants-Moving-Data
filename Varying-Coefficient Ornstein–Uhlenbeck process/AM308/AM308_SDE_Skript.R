...
track <- subset(data, ID == unique(ID)[11]) # keep AM308 only
...
keep_rows_2 <- which(track$date > as.POSIXct("2008-10-01 00:00:00") &
                       track$date < as.POSIXct("2009-04-15 23:59:59"))
...
# Confidence bands
quants <- apply(post_nu, 1, quantile, probs = c(0.025, 0.975))
new_data <- data.frame(temp = seq(from = 15, to = 43, length.out = 1000))
df <- data.frame(x = new_data$temp,
                 mle = nu,
                 low = quants[1,],
                 upp = quants[2,])
