...
mod.speed = bam(v ~ s(temp,  k = 40, bs="ps")+
                  season+
                  s(id, bs = "re")+
                  s(hour,k=50, bs= "cp"),
...
grid.arrange(arrangeGrob(gaussian, top = 'Gaussian'), arrangeGrob(gamma, top = 'Gamma'), ncol=2)
...
pacf(res_gam, ylim=c(0,1),main ="Gamma")
...
fig_speed_temp =
  ...
  geom_rangeframe(data = data_frame(x=c(15,40), y = c(0.15,0.75)),aes(x,y))+
  geom_smooth(aes(x = temp, y = pred.mean*1/1e3, 
                  col = season, fill = season, lty = season), 
              alpha = 0.2, lwd = 0.5)+
  geom_pointrange(aes(x = temp, y = v.mean*1/1e3, 
